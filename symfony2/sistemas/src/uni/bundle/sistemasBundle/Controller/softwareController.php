...
use uni\bundle\sistemasBundle\Entity\software;
use uni\bundle\sistemasBundle\Form\softwareType;
...
class softwareController extends Controller
{
     ...
     public function buscarSWAction()
    {

        $nomSW=null;
        return $this->render('uniSistemasBundle:software:buscarsw.html.twig', array(
            'nomSW' => $nomSW,            
        ));
    }
    
    public function respBuscSWAction(Request $res)
    { 
        $nom= $res->request->get('nomsoft'); // Modo symfony2
         
        $em = $this->getDoctrine()->getManager();
        
//        $dql = "select s from uniSistemasBundle:software s where s.nombre like :nombre";
//        $query = $em->createQuery($dql);
//        $query->setParameter('nombre', '%'.$nom.'%');
//        $entities = $query->getResult();
        
        $entity = $em->getRepository('uniSistemasBundle:software')->  findOneByNombre($nom);       
